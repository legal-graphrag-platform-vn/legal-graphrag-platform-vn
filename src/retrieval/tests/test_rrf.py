...
def test_rrf_fusion_ordering():
    ...
    vector_results[0], vector_results[1] = (
        vector_results[1],
        vector_results[0],
    )  # Đổi A và B -> Vector: B, A, C
    ...
    # Fusion must not mutate channel lists used by the final RRF stage.
    assert vector_results[0].final_score is None
    assert bm25_results[0].final_score is None
