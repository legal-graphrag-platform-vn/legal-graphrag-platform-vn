...
def test_extract_body_lines_keeps_full_document_but_removes_trailing_table_of_contents() -> (
    None
):
    body = """Trang chủ
Tải về
...
def test_extract_body_lines_removes_table_of_contents_without_navigation_marker() -> (
    None
):
    body = """QUỐC HỘI
Điều 1. Phạm vi điều chỉnh
...
def test_extract_body_lines_preserves_appendix() -> None:
    body = """Tải về
Điều 1. Phạm vi điều chỉnh
Nội dung điều luật.
PHỤ LỤC I
1. Dòng có cấu trúc giống khoản
a) Dòng có cấu trúc giống điểm
"""

    assert _extract_body_lines(body)[-3:] == [
        "PHỤ LỤC I",
        "1. Dòng có cấu trúc giống khoản",
        "a) Dòng có cấu trúc giống điểm",
    ]
...
def test_crawl_by_search_mocked(
    mock_crawl_and_save: MagicMock, mock_sync_playwright: MagicMock
) -> None:
    ...
    mock_crawl_and_save.assert_called_once_with(
        "https://vbpl.vn/van-ban/chi-tiet/ldn2020", "L59_2020", "59/2020/QH14", raw_dir
    )
