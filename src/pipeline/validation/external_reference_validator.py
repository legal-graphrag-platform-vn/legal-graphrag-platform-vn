...
def validate_external_reference_bundle(
    ...
    expected_target_count = len(checkpoints)
    ...
    for checkpoint in checkpoints:
        ...
        if resolution.status != "RESOLVED":
            ...
            continue
        if resolution.reference_scope != "EXTERNAL" or resolution.is_self_reference:
            errors.append(
        ...
        if evidence_tuple != receipt_tuple or state_tuple != receipt_tuple:
            errors.append(
        ...
        if tuple(reference.target_unit_ids) != tuple(resolution.target_ids):
            ...
            continue
        if len(reference.target_unit_ids) != 1:
            errors.append(
                f"{checkpoint.reference_bundle_id}: each checkpoint must prove one target"
            )
            continue
        ...
        if actual_source != (source_type, source_document_id, source_ancestors):
            errors.append(f"{checkpoint.reference_bundle_id}: source evidence mismatch")
            continue
        if actual_target != (target_type, target_document_id, target_ancestors):
            errors.append(f"{checkpoint.reference_bundle_id}: target evidence mismatch")
            continue
