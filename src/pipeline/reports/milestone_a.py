...
def generate_milestone_a_report(raw_doc_code: str, reports_dir: Path) -> dict[str, Any]:
    ...
    write_idempotent = all(
        write_1.get(key) == write_2.get(key) for key in idempotency_keys
    )
    projection_match = all(
        snapshot.get("projection_match")
        and snapshot.get("payload_projection_sha256")
        == snapshot.get("graph_projection_sha256")
    ...
    embedding_labels = (
        ("Appendix", "Article", "Clause")
        if "Appendix" in coverage
        else ("Article", "Clause")
    )
    embedding_pass = all(
        coverage.get(label, {}).get("coverage") == 1.0 for label in embedding_labels
    )
    ...
    graph_quality_pass = (
        all(
            ...
            for key in (
                ...
                "duplicate_relation_identity_count",
    ...
    report = {
        ...
        "semantic_edge_accounting": {
            ...
            "excluded_edge_count_by_type": semantic_graph.get(
                "excluded_edge_count_by_type", {}
            ),
...
def write_milestone_a_report(
    ...
    lines = [
        ...
        *[
            f"- {query_id} relevant@5: {'PASS' if passed else 'FAIL'}"
            for query_id, passed in vector["query_relevant_at_5"].items()
        ],
        *[
            f"- {index_name} relevant@5: {'PASS' if passed else 'FAIL'}"
            for index_name, passed in vector["index_relevant_at_5"].items()
        ],
...
def _validate_vector_evidence(
    results: dict[str, Any], judgements: list[dict[str, Any]]
) -> dict[str, Any]:
    expected_keys = {
    ...
    query_pass = {
        query_id: any(
            item.get("query_id") == query_id and item.get("judgement") == "relevant"
    ...
    index_pass = {
        index_name: any(
            item.get("index_name") == index_name and item.get("judgement") == "relevant"
            for item in complete
        )
    ...
    return {
        ...
        "pass": keys_match
        and len(complete) == len(expected_keys) == 30
        and all(query_pass.values())
        and all(index_pass.values()),
    }
