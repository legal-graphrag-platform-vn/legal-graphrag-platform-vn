...
| C2-4 | `AMENDED_BY` cross-level (Article→Clause) được traverse đúng | Test query với văn bản đã có sửa đổi cross-level |
...
| C2-7 | `IMPLEMENTED_BY` resolve được Law→Circular (direct) | Test query với văn bản có Luật giao thẳng cho Thông tư |
...
| C6-5 | 12 ADRs trong `00_architecture_decisions.md` đã có justification | Dùng làm reference khi bảo vệ |
