...
## 5. Tại sao dùng BM25 + Vector (Hybrid) thay vì chỉ Vector?

*Xem chi tiết: ADR-11*

**Bối cảnh**: Thiết kế Retriever để tìm kiếm `Clause` dựa trên câu hỏi người dùng.
- **Tại sao chọn Hybrid**: Ngôn ngữ pháp lý chứa rất nhiều "từ khóa chính xác" (ví dụ: mã số văn bản `59/2020/QH14`, thuật ngữ cụ thể như `CTCP`). Vector search (ngữ nghĩa) thường bỏ sót các keyword chính xác này, trong khi BM25 (keyword matching) lại rất mạnh. Kết hợp cả hai (Hybrid Retrieval) mang lại Recall cao nhất.
